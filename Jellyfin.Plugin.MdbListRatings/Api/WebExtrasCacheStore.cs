...
internal sealed class WebExtrasCacheStore
{
    ...
    {
        public WebExtrasCachedData Data { get; set; } = new();
    }

    internal sealed class WebExtrasCachedData
    {
        public DateTimeOffset? RottenTomatoesCachedAtUtc { get; set; }
        public bool HasRtCriticsCertified { get; set; }
        public bool RtCriticsCertified { get; set; }
        public bool HasRtAudienceVerified { get; set; }
        public bool RtAudienceVerified { get; set; }

        public DateTimeOffset? AniListCachedAtUtc { get; set; }
        public bool HasAniListScore { get; set; }
        public int? AniListScore { get; set; }
    }

    public WebExtrasCacheStore(string cacheDir, ILogger logger)
    {
        _cacheDir = cacheDir;
        _logger = logger;
    ...
    public async Task<CacheEnvelope?> TryGetAsync(string key, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to read web-extras cache entry: {Key}", key);
            return null;
        }
    }
    ...
    public async Task SaveAsync(string key, CacheEnvelope env, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to write web-extras cache entry: {Key}", key);
            try
            {
                if (File.Exists(tmp))
                {
